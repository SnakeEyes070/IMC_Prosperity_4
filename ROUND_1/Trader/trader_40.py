# trader.py - Multi-Day Trend Reset (Target: 10,000+)
...
class Trader:
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        ENDGAME_TS = 99000
        TREND_RATE = 0.001001
        DAY_GAP_THRESHOLD = 100  # Pepper jumps >100 points between days
        ...
        if PEPPER in state.order_depths:
            ...
            if od.buy_orders and od.sell_orders:
                ...
                # Detect new day by large price gap
                ...
                # Check for day boundary: price jumped > DAY_GAP_THRESHOLD since last tick
                if abs(mid - data[last_mid_key]) > DAY_GAP_THRESHOLD:
                    # New day detected! Reset anchor and position tracking
                    data[anchor_key] = mid
                    data[anchor_ts_key] = ts
                    # We don't reset position here—the exchange does that automatically.
                    # But we should aggressively re-enter max long.
                ...
                if ts >= ENDGAME_TS:
                    ...
                    # Aggressively go max long immediately
                    if buy_cap > 0:
                        ...
                        # Also place a tight passive bid to catch any dips
                        if buy_cap > 0:
                            passive_bid = int(fair - 1)  # Even tighter
                            if passive_bid < best_ask:
                                qty2 = min(buy_cap, 15)
                                orders.append(Order(PEPPER, passive_bid, qty2))

                    # Trim only if wildly above fair (unlikely in trend)
                    sell_cap = limit + pos
        ...
        OSM_REV_ENTRY   = 8
        OSM_REV_SIZE    = 25
        OSM_UNWIND_FRAC = 0.6  # Unwind earlier to free capital
