# trader.py - Final Optimized Round 1 Strategy (Stable 5,000+)
...
class Trader:
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        # =========================================================================
        # PROVEN CONFIGURATION – DO NOT CHANGE WITHOUT BACKTESTING
        # =========================================================================
        CONFIG = {
            "ASH_COATED_OSMIUM": {
                "fair_initial": 9984,
                "ema_alpha": 0.1,
                "mm_offset": 3,             # Tight – Osmium range is stable
                "mm_size": 16,              # Aggressive but safe with stop-loss
                "max_size": 24,             # Cap for high‑confidence breakout
                "mean_rev_thresh": 60,      # Fade only significant dips
                "stop_loss_ticks": 8,       # Cut losers early
                "momentum_thresh": 2.2,     # Breakout sensitivity
                "imbalance_thresh": 0.25,   # Volume imbalance filter
                "volatility_window": 20,
                "levels": 2,                # Two‑level quote ladder
                "max_spread": 20            # Safety – skip if spread too wide
            },
            "INTARIAN_PEPPER_ROOT": {
                "fair_initial": 11479,
                "ema_alpha": 0.08,
                "mm_offset": 5,             # Wider for volatile asset
                "mm_size": 12,              # Moderate size
                "max_size": 18,
                "trend_bias": True,         # Avoid fading a strong trend
                "trend_thresh": 50,
                "mean_rev_thresh": 350,     # Wide – only fade extreme moves
                "momentum_thresh": 2.8,
        ...
        for product, od in state.order_depths.items():
            ...
            # ----- 7. DYNAMIC SIZE (Capacity Scaled) -----
            buy_cap = limit - pos
            sell_cap = limit + pos
            ...
            # ----- 12. CORE MARKET MAKING (Multi‑Level Ladder) -----
            if not orders:
