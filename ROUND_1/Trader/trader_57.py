# trader.py - Recalibrated (Target: 6,500–7,000)
import json
import math
...
class Trader:
    ...
    # ── PEPPER ────────────────────────────────────────────────────────────
    PEPPER_SLOPE   = 0.001
    PEPPER_BUY_TOL = 15             # back to proven value

    # ── OSMIUM ────────────────────────────────────────────────────────────
    OSM_FAIR_FALLBACK = 10_000
    OSM_EMA_ALPHA    = 0.015        # back to 0.015
    OSM_L1_OFFSET    = 4            # back to 4 (3 was too tight)
    OSM_L2_OFFSET    = 7
    OSM_L3_OFFSET    = 11
    OSM_L1_SIZE      = 15
    OSM_L2_SIZE      = 12
    OSM_L3_SIZE      = 8
    OSM_MR_THRESH    = 7            # back to 7 (6 was too aggressive)
    OSM_MR_MAX_QTY   = 20
    OSM_SKEW_FACTOR  = 0.06         # restored price skew (zero skew hurt)

    # ── TIMING ────────────────────────────────────────────────────────────
    ROUND_DAYS       = 3
    MAX_TS           = 99_900
    ENDGAME_START    = 97_000       # keep earlier unwind (proven beneficial)
