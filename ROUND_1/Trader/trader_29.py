# trader.py - Last Stand (All or Nothing)
...
class Trader:
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        CONFIG = {
            "ASH_COATED_OSMIUM": {"offset": 1, "size": 45, "fair": 10000},
            "INTARIAN_PEPPER_ROOT": {"offset": 2, "size": 40, "fair": 11480}
        }
        ...
        for product, od in state.order_depths.items():
            ...
            # Aggressive market making + taking
            if best_ask < fair - 2:
                qty = min(-od.sell_orders.get(best_ask, 0), limit - pos, size)
                if qty > 0: orders.append(Order(product, best_ask, qty))
            elif best_bid > fair + 2:
                qty = min(od.buy_orders.get(best_bid, 0), limit + pos, size)
                if qty > 0: orders.append(Order(product, best_bid, -qty))
            else:
                buy_cap = limit - pos
                sell_cap = limit + pos
