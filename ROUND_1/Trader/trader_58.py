# trader.py - Precision Calibrated (Target: 7,000–7,500)
import json
import math
...
class Trader:
    ...
    PEPPER_SLOPE   = 0.001
    PEPPER_BUY_TOL = 12             # tighter: avoids overpaying

    OSM_FAIR_FALLBACK = 10_000
    OSM_EMA_ALPHA    = 0.015
    OSM_L1_OFFSET    = 3            # tighter: becomes best quote
    OSM_L2_OFFSET    = 7
    OSM_L3_OFFSET    = 11
    OSM_L1_SIZE      = 18           # larger: more volume on tightest level
    OSM_L2_SIZE      = 12
    OSM_L3_SIZE      = 8
    OSM_MR_THRESH    = 5            # more aggressive mean reversion
    OSM_MR_MAX_QTY   = 25           # larger take size
    OSM_SKEW_FACTOR  = 0.06

    ROUND_DAYS       = 3
    MAX_TS           = 99_900
    ENDGAME_START    = 96_000       # earlier unwind
