# trader.py - Rook‑E1 Calibrated Final (Target: 7,000–8,000)
import json
import math
...
class Trader:
    ...
    # ── PEPPER (Trend Commitment) ──────────────────────────────────────────
    PEPPER_SLOPE   = 0.001          # confirmed linear trend
    PEPPER_BUY_TOL = 18             # wider to guarantee instant fill

    # ── OSMIUM (Strategic Orders) ──────────────────────────────────────────
    OSM_FAIR_FALLBACK = 10_000
    OSM_EMA_ALPHA    = 0.01         # slower EMA (less noise)
    OSM_L1_OFFSET    = 3            # tighter: we become the best quote
    OSM_L2_OFFSET    = 5
    OSM_L3_OFFSET    = 8
    OSM_L1_SIZE      = 14
    OSM_L2_SIZE      = 12
    OSM_L3_SIZE      = 8
    OSM_MR_THRESH    = 6            # more aggressive mean reversion
    OSM_MR_MAX_QTY   = 20
    OSM_SKEW_FACTOR  = 0.0          # NO price skew – only size adjustment

    # ── TIMING (Auction Dynamics) ──────────────────────────────────────────
    ROUND_DAYS       = 3
    MAX_TS           = 99_900
    ENDGAME_START    = 97_000       # earlier unwind = smoother exit
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # PEPPER – Volume‑Weighted Anchor & Aggressive Accumulation
    # ═══════════════════════════════════════════════════════════════════════
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # OSMIUM – Micro‑Price, Imbalance Filter, Zero Price Skew
    # ═══════════════════════════════════════════════════════════════════════
    ...
    def _osmium_orders(self, state: TradingState, data: dict) -> List[Order]:
        ...
        if best_bid is not None and best_ask is not None:
            ...
            raw_mid = float(best_bid) + self.OSM_L1_OFFSET
        elif best_ask is not None:
            raw_mid = float(best_ask) - self.OSM_L1_OFFSET
        else:
            raw_mid = float(self.OSM_FAIR_FALLBACK)

        ema  = data.get("osm_ema", float(self.OSM_FAIR_FALLBACK))
        ema += self.OSM_EMA_ALPHA * (raw_mid - ema)
        data["osm_ema"] = ema
        fair = round(ema)
        ...
        # Order book imbalance (Tipping the Scale)
        ...
        # Passive Ladder – Zero Price Skew, Size Adjustment Only
        levels = [
        ...
        for offset, base_size in levels:
            ...
            buy_size  = max(1, round(base_size * (1.0 - max(0.0,  long_bias) * 0.6)))
            sell_size = max(1, round(base_size * (1.0 - max(0.0, -long_bias) * 0.6)))

            # Imbalance Filter: skip if pressure is against us
