# trader.py - Final Round 1 Optimization (Target: 1500+)
...
class Trader:
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        # Aggressive for Osmium, trend-aware for Pepper
        CONFIG = {
            "ASH_COATED_OSMIUM": {
                "base_offset": 6,      # Tighter (was 8)
                "base_size": 9,        # Larger (was 7)
                "ema_alpha": 0.1,
                "trend_bias": False
            },
            "INTARIAN_PEPPER_ROOT": {
                "base_offset": 9,      # Still wide
                "base_size": 6,        # Slightly larger
                "ema_alpha": 0.05,     # Very slow EMA for trend detection
                "trend_bias": True     # Mild trend following
            }
        }
        ...
        for product, od in state.order_depths.items():
            ...
            if cfg.get("trend_bias", False):
                trend = mid - fair
                if trend > 20:   # Uptrend
                    buy_offset = max(4, offset - 1)   # Buy dips more aggressively
                    sell_offset = offset + 1          # Sell less eagerly
                elif trend < -20: # Downtrend
                    buy_offset = offset + 1
                    sell_offset = max(4, offset - 1)
