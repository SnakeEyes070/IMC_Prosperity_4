# trader.py - v3 Optimized for 6,000+ (Enhanced Accumulation & Tighter Spreads)
...
class Trader:
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        # =====================================================================
        # INTARIAN PEPPER ROOT — Maximum Aggression Accumulation
        # =====================================================================
        if PEPPER in state.order_depths:
            ...
            if od.buy_orders and od.sell_orders:
                ...
                if ts >= ENDGAME_TS:
                    ...
                    if buy_cap > 0:
                        # Aggressive lifting: up to 30 units per tick (increased from 20)
                        qty = min(buy_cap, ask_vol, 30)
                        if qty > 0:
                            orders.append(Order(PEPPER, best_ask, qty))
                            buy_cap -= qty

                        # Enhanced passive bid (size 15, increased from 10)
                        if buy_cap > 0:
                    ...
                    # Trim only if wildly above fair (threshold 20 remains safe)
                    sell_cap = limit + pos
        ...
        # =====================================================================
        # ASH_COATED_OSMIUM — Tighter Spreads & Larger Mean Reversion
        # =====================================================================
        OSM_FAIR        = 10000.0
        OSM_MM_OFFSET   = 3       # Tighter (was 4)
        OSM_MM_SIZE     = 14      # Larger (was 12)
        OSM_REV_ENTRY   = 8       
        OSM_REV_SIZE    = 24      # Larger (was 20)
        OSM_UNWIND_FRAC = 0.7
                ...
                if ts >= ENDGAME_TS:
                    ...
                    if best_ask <= fair - OSM_REV_ENTRY:
                        ...
                        for level in range(2):
                            offset = OSM_MM_OFFSET + level * 4  # 3 and 7
